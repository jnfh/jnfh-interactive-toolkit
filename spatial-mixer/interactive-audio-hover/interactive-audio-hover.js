...
class InteractiveAudioHover {
    ...
    initCanvas() {
        ...
        if (typeof this.canvas.getContext !== 'function') {
            throw new Error('Canvas element is invalid - getContext method not available');
        }
        ...
        const resizeCanvas = () => {
            ...
            if (!this.canvas) {
                console.warn('⚠️ Canvas not available during resize');
                return;
            }
    ...
    setupEventListeners() {
        ...
        // Double-check canvas is still valid before each operation
        if (!this.canvas || !this.canvas.addEventListener) {
            console.warn('⚠️ Canvas invalid, skipping event listeners');
            return;
        }
        ...
        if (this.canvas && this.canvas.addEventListener) {
            try {
                ...
                console.warn('⚠️ Error setting up canvas event listeners:', error);
            }
        }

        // Control buttons (only if they exist - not in embed-player.html)
        ...
        // File input (only if exists - not in embed-player.html)
        ...
        // Master volume (only if exists - not in embed-player.html)
        ...
        // Fade radius (only if exists - not in embed-player.html)
        ...
        // Fade speed (only if exists - not in embed-player.html)
    ...
    draw() {
        ...
        // Clear canvas first (important for transparent backgrounds)
    ...
    updateActiveCount() {
        try {
            ...
            if (activeCountEl && typeof activeCountEl.textContent !== 'undefined') {
                const activeCount = this.sources.filter(s => s.isPlaying && s.targetVolume > 0.01).length;
                if (activeCountEl && activeCountEl.textContent !== undefined) {
                    activeCountEl.textContent = activeCount;
                }
            }
        } catch (error) {
            // Silently fail if element doesn't exist (e.g., in embed-player.html)
            // console.warn('updateActiveCount error:', error);
        }
    }
    ...
    updateSourceInfo() {
        try {
            ...
            // Silently fail if element doesn't exist (e.g., in embed-player.html)
            // console.warn('updateSourceInfo error:', error);
        }
    }
    ...
    updateMousePos() {
        try {
            ...
            if (!mousePosEl || typeof mousePosEl.textContent === 'undefined') {
                return; // Element doesn't exist (e.g., in embed-player.html)
            }
            
            const posText = this.mousePos.x >= 0 && this.mousePos.y >= 0
                ? `${Math.round(this.mousePos.x)}, ${Math.round(this.mousePos.y)}`
                : '-';
            if (mousePosEl && mousePosEl.textContent !== undefined) {
                mousePosEl.textContent = posText;
            }
        } catch (error) {
            // Silently fail if element doesn't exist (e.g., in embed-player.html)
            // console.warn('updateMousePos error:', error);
        }
    }
    ...
    async importConfiguration(configJson, audioFiles = null) {
        try {
            ...
            if (config.sources && config.sources.length > 0) {
                ...
                console.log(`Sources cleared. Starting parallel import of ${config.sources.length} sources...`);
                
                // Load all audio buffers in parallel for faster loading
                const loadAudioBuffer = async (sourceConfig, index) => {
                    let audioBuffer;
                    
                    // Priority 1: Load from URL if available (best for large files)
                    if (sourceConfig.audioUrl) {
                        try {
                            console.log(`[${index + 1}/${config.sources.length}] Loading audio from URL: ${sourceConfig.audioUrl}`);
                            ...
                            console.log(`✓ [${index + 1}/${config.sources.length}] Audio loaded from URL`);
                        } catch (error) {
                    ...
                    if (!audioBuffer && sourceConfig.audioData) {
                        try {
                            ...
                            console.log(`✓ [${index + 1}/${config.sources.length}] Audio loaded from embedded Base64 data`);
                        } catch (error) {
                    ...
                    if (!audioBuffer && audioFiles && audioFiles[index]) {
                        try {
                            const arrayBuffer = await audioFiles[index].arrayBuffer();
                            audioBuffer = await this.audioContext.decodeAudioData(arrayBuffer);
                            console.log(`✓ [${index + 1}/${config.sources.length}] Audio loaded from provided file`);
                        } catch (error) {
                            console.error(`Error loading audio file:`, error);
                        }
                    }
                    
                    return { sourceConfig, audioBuffer, index };
                };
                
                // Load all audio buffers in parallel
                const loadPromises = config.sources.map((sourceConfig, i) => loadAudioBuffer(sourceConfig, i));
                const loadedSources = await Promise.all(loadPromises);
                
                console.log(`✓ All ${loadedSources.length} audio buffers loaded. Adding sources...`);
                
                // Now add all sources sequentially (to maintain order and avoid race conditions)
                for (let i = 0; i < loadedSources.length; i++) {
                    const { sourceConfig, audioBuffer, index } = loadedSources[i];
                    
                    if (!audioBuffer) {
                        console.warn(`No audio data available for source ${index + 1} (${sourceConfig.name}), skipping`);
                        continue;
                    }
                    
                    // Create source with saved position and settings
                    try {
                        const addedSource = await this.addAudioSourceFromConfig(sourceConfig, audioBuffer);
                        
                        if (!addedSource) {
                            console.error(`❌ addAudioSourceFromConfig returned null/undefined for source ${index + 1} (${sourceConfig.name})`);
                            continue;
                        }
                        ...
                        console.log(`✓ Source ${index + 1}/${config.sources.length} added successfully. Total sources: ${this.sources.length}`);
                    } catch (sourceError) {
                        console.error(`❌ Failed to add source ${index + 1} (${sourceConfig.name}):`, sourceError);
                        // Continue with next source instead of failing completely
                    }
                }
                
                console.log(`✓ Import complete: ${this.sources.length} sources imported successfully`);
                
                // Force a redraw and update after import
                if (this.sources.length > 0) {
                    this.draw();
                    this.updateSourceCount();
                    console.log(`✓ Canvas redrawn with ${this.sources.length} sources`);
                }
            }
    ...
    async addAudioSourceFromConfig(sourceConfig, audioBuffer) {
        console.log(`[addAudioSourceFromConfig] Starting for source: ${sourceConfig.name}`);
        console.log(`[addAudioSourceFromConfig] audioBuffer provided: ${!!audioBuffer}`);
        console.log(`[addAudioSourceFromConfig] audioContext exists: ${!!this.audioContext}`);
        
        if (!this.audioContext) {
            console.log(`[addAudioSourceFromConfig] AudioContext missing, initializing...`);
            await this.initAudioContext();
            console.log(`[addAudioSourceFromConfig] AudioContext initialized`);
        }

        console.log(`[addAudioSourceFromConfig] About to enter try block`);
        try {
            // Verify audioContext still exists
            if (!this.audioContext) {
                throw new Error('AudioContext is null after initialization check');
            }
            
            console.log(`[addAudioSourceFromConfig] AudioContext state: ${this.audioContext.state}`);
            console.log(`[addAudioSourceFromConfig] AudioBuffer exists: ${!!audioBuffer}`);
            
            // Skip resume attempt during import - it requires user interaction anyway
            // AudioContext will be resumed when user interacts with the page (handled in player.html)
            console.log(`[addAudioSourceFromConfig] AudioContext state: ${this.audioContext.state} (skipping resume - will resume on user interaction)`);
            
            // Verify audioContext still exists
            if (!this.audioContext) {
                console.error(`[addAudioSourceFromConfig] ERROR: AudioContext became null`);
                throw new Error('AudioContext became null');
            }
            
            // Verify audioContext still exists after resume attempt
            if (!this.audioContext) {
                console.error(`[addAudioSourceFromConfig] ERROR: AudioContext became null after resume attempt`);
                throw new Error('AudioContext became null after resume attempt');
            }
            
            // Verify audioBuffer exists
            if (!audioBuffer) {
                console.error(`[addAudioSourceFromConfig] ERROR: audioBuffer is null or undefined`);
                throw new Error('audioBuffer is null or undefined');
            }
            
            console.log(`[addAudioSourceFromConfig] Creating source object for: ${sourceConfig.name}`);
            ...
            // Verify physics properties are set (only log first source to avoid spam)
            if (this.sources.length === 0) {
            ...
            if (!this.masterGainNode) {
                console.warn('⚠️ masterGainNode missing, creating it now');
                this.masterGainNode = this.audioContext.createGain();
                this.masterGainNode.gain.value = this.masterVolume || 0.5;
                this.masterGainNode.connect(this.audioContext.destination);
            }
            ...
            console.log(`[addAudioSourceFromConfig] Checking for duplicates. Current sources count: ${this.sources.length}`);
            
            // Check for duplicate before adding (by name+position, not by ID since IDs are generated fresh)
            ...
            if (existingSource) {
                console.warn(`⚠️ Duplicate source detected: ${source.name} at (${source.x}, ${source.y}), skipping. Current count: ${this.sources.length}`);
                return null;
            }
            
            console.log(`[addAudioSourceFromConfig] No duplicate found. Adding source. Before push: ${this.sources.length} sources`);
            
            // Verify we're not exceeding expected count
            const expectedCount = this.sources.length + 1;
            this.sources.push(source);
            
            console.log(`[addAudioSourceFromConfig] After push: ${this.sources.length} sources (expected: ${expectedCount})`);
            
            if (this.sources.length !== expectedCount) {
                console.error(`❌ Source count mismatch! Expected ${expectedCount}, got ${this.sources.length}`);
            }
            
            console.log(`✓ Source "${source.name}" added successfully. Total sources: ${this.sources.length}`);
            this.updateSourceCount();
            this.updateSourceEditor();
            this.draw();
            ...
            console.log(`[addAudioSourceFromConfig] Successfully completed for: ${sourceConfig.name}`);
            return source; // Return the added source
        } catch (error) {
            console.error(`❌ Error adding audio source "${sourceConfig.name}" from config:`, error);
            console.error('Error type:', error?.constructor?.name);
            console.error('Error message:', error?.message);
            console.error('Error stack:', error?.stack);
            console.error('Source config name:', sourceConfig?.name);
            console.error('AudioBuffer exists:', !!audioBuffer);
            console.error('AudioContext exists:', !!this.audioContext);
    ...
    updateUIControls() {
        ...
        if (fadeRadiusSlider) {
            ...
            const fadeRadiusValue = document.getElementById('fadeRadiusValue');
            if (fadeRadiusValue) {
                fadeRadiusValue.textContent = this.fadeRadius + 'px';
            }
        }
        ...
        if (fadeSpeedSlider) {
            ...
            const fadeSpeedValue = document.getElementById('fadeSpeedValue');
            if (fadeSpeedValue) {
                fadeSpeedValue.textContent = this.fadeSpeed + 's';
            }
        }
        ...
        if (physicsStrengthSlider) {
            ...
            const physicsStrengthValue = document.getElementById('physicsStrengthValue');
            if (physicsStrengthValue) {
                physicsStrengthValue.textContent = Math.round(this.physicsStrength * 100) + '%';
            }
        }
        ...
        if (driftStrengthSlider) {
            ...
            const driftStrengthValue = document.getElementById('driftStrengthValue');
            if (driftStrengthValue) {
                driftStrengthValue.textContent = Math.round(this.driftStrength * 100) + '%';
            }
        }
        ...
        if (mouseRepulsionSlider) {
            ...
            const mouseRepulsionValue = document.getElementById('mouseRepulsionValue');
            if (mouseRepulsionValue) {
                mouseRepulsionValue.textContent = Math.round(this.mouseRepulsion * 100) + '%';
            }
        }
        ...
        if (sourceRepulsionSlider) {
            ...
            const sourceRepulsionValue = document.getElementById('sourceRepulsionValue');
            if (sourceRepulsionValue) {
                sourceRepulsionValue.textContent = Math.round(this.sourceRepulsion * 100) + '%';
            }
        }
        ...
        if (orbitalStrengthSlider) {
            ...
            const orbitalStrengthValue = document.getElementById('orbitalStrengthValue');
            if (orbitalStrengthValue) {
                orbitalStrengthValue.textContent = Math.round(this.orbitalStrength * 100) + '%';
            }
        }
        ...
        if (orbitalSpeedSlider) {
            ...
            const orbitalSpeedValue = document.getElementById('orbitalSpeedValue');
            if (orbitalSpeedValue) {
                orbitalSpeedValue.textContent = Math.round((this.orbitalSpeed / 0.1) * 100) + '%';
            }
        }
    }
